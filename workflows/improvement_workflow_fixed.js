...
2. Yield each token as an SSE event: "data: TOKEN_HERE\\n\\n"
...
1. Defines 10-15 common query templates as a list of dicts with patterns, cypher, and answer_template fields.
...
4. Modify the chat() method: try _match_template first. If matched, use the template's cypher directly (skip LLM Cypher generation). If not matched, fall back to LLM-based Cypher generation.

Example templates to include:
- Brand queries: "有哪些品牌", "什么品牌", "品牌列表"
- Category queries: "分类", "category"
- Brand+Product: "(品牌名)有什么", "(品牌名)的产品"
- Category filtering: "分类.*商品", "分类下"
- Price queries: "多少钱", "价格"
- SKU queries: "规格", "SKU"
- Attribute queries: "颜色", "尺码", "材质"
- Count queries: "有多少", "多少个"
- Relationship queries: "属于哪个品牌", "属于哪个分类"
...
1. Brand query: "有哪些手机品牌？" -> MATCH (t:BaseTrademark) RETURN t.name
2. Category query: "有哪些商品分类？" -> MATCH (c:Category1) RETURN c.name
3. Brand+Category combo: "华为手机有哪些？" -> MATCH (s:SPU)-[:Belong]->(t:BaseTrademark), (s)-[:Belong]->(c:Category3) WHERE t.name CONTAINS '华为' AND c.name CONTAINS '手机' RETURN s.name
4. Attribute query: "有哪些颜色的衣服？" -> MATCH (s:SPU)-[:Have]->(a:SaleAttr)-[:Have]->(v:SaleAttrValue) WHERE v.name CONTAINS '颜色' RETURN s.name
5. Relationship query: "XX商品属于哪个品牌？" -> MATCH (s:SPU)-[:Belong]->(t:BaseTrademark) WHERE s.name CONTAINS 'XX' RETURN t.name
6. SKU query: "XX商品有哪些规格？" -> MATCH (sku:SKU)-[:Belong]->(s:SPU) WHERE s.name CONTAINS 'XX' RETURN sku.name
...
1. Add a microphone button next to the send button:
   - Style: circular button with mic icon (unicode mic character)
...
3. Add graceful fallback:
   ...
   - If not available, hide the mic button or show tooltip

4. Handle errors:
   - 'not-allowed': show permission error message
   - 'no-speech': show no speech detected message
